...
// includes the accumulated offset and not processed update.
func TestProgressReporterReportDone(t *testing.T) {
	synctest.Test(t, func(t *testing.T) {
		...
		time.Sleep(2 * interval)
		r.reportRead(read2)
		assert.NotEmpty(t, consumeNonblocking(channel), "should sent after interval elapses")
		r.reportRead(read3)
...
// TestNoopProgressReporter a noopProgressReporter
